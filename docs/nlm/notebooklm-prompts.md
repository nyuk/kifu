...
`지금은 v1.0 기준으로 먼저 설명해줘. v1.1은 "변경/확장"으로만 부록에 정리해줘.  
출력은 반드시 `[v1.0 기준]`, `[v1.1 변경사항]`, `[충돌/오해 포인트]` 3섹션만 사용하고,  
근거는 먼저 v1.0 문서(architecture/summary-pack/runs/handlers), 그다음 v1.1 문서(ADR/스펙 갱신/테스트) 순서로 제시해줘.`

## 충돌 해소 규칙
- v1.0에서 설명되지 않은 동작은 `[v1.1 변경사항]`으로만 이동
- v1.1에 추가된 필드/응답은 본문보다 부록에 우선 배치
- 문장 시작에 버전 태그 생략 금지 (`[v1.0 기준]`, `[v1.1 변경사항]` 사용)
