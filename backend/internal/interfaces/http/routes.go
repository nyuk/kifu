...
func RegisterRoutes(
	...
	authHandler := handlers.NewAuthHandler(userRepo, userIdentityRepo, refreshTokenRepo, subscriptionRepo, jwtSecret, growthService)
	userHandler := handlers.NewUserHandler(userRepo, subscriptionRepo)
	exchangeHandler := handlers.NewExchangeHandler(exchangeRepo, tradeRepo, encryptionKey, exchangeSyncer, runRepo, growthService)
	...
	importHandler := handlers.NewImportHandler(portfolioRepo, runRepo, growthService)
	connectionHandler := handlers.NewConnectionHandler()
	safetyHandler := handlers.NewSafetyHandler(safetyRepo)
	guidedReviewHandler := handlers.NewGuidedReviewHandler(guidedReviewRepo, growthService)
	...
	marketing.Get("/workspace", marketingHandler.GetWorkspace)
